  ...
  return {
    ...
    url:
      ...
      img.url
        ...
        .replace('.jpg', img.alpha_channel ? '.png' : '.jpg'),
    thumb:
      'https:' +
      img.url
        .replace('t_thumb', sizeThumb)
        .replace('.jpg', img.alpha_channel ? '.png' : '.jpg'),
  };
};
...
export class IGBDProvider extends BaseProvider<IGDBGamesResult[]> {
  ...
  async convertToSearchResults(
    data: IGDBGamesResult[],
    count = 51,
    ...
    return {
      ...
      results: games.map(
        ({
          ...
          if (screenshots) {
            result.screenshots = screenshots.map((data) =>
              extractUsefulImage(data),
            );
          ...
          if (platforms) {
            result.platforms = platforms.map(
              ({ id, abbreviation, platform_logo, name }) => {
                return {
                  id,
                  abbreviation,
                  name,
                  logos: [extractUsefulImage(platform_logo)],
                };
              },
            );
          }
          if (involved_companies) {
            result.involved_companies = involved_companies.map(
              ({ id, company: { logo, ...restOfCompany } }) => ({
                id,
                company: {
                  ...restOfCompany,
  ...
  async convertToPlatformsResults(
    data: IGDBPlatformsResult[],
    count: number = 0,
    ...
    return {
      ...
      results: data.map(
        ({
          ...
          abbreviation,
          versions: versions
            ? versions.map(({ id, name, abbreviation, platform_logo }) => ({
                id,
                name,
                abbreviation,
                ...(platform_logo
  ...
  async converToCompaniesResults(
    data: IGDBMultiQueryWithCount<IGDBCompanyResult[]>,
  ): Promise<unknown> {
    ...
    return {
      count,
      results: companies
        ? companies.map(({ id, name, description, logo }) => ({
            id,
            name,
            description,
            ...(logo ? { logo: extractUsefulImage(logo) } : {}),
